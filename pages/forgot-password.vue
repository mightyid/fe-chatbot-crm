...
const getOtp = async () => {
  return await $api('user/auth/forgot-password', {
    method: 'POST',
...
const verifyOtp = async () => {
  const { result } = await $api<ResultOtpType>('user/auth/verify-otp', {
    method: 'POST',
    body: {
...
const resetPassword = handleSubmit(async () => {
  const { result, statusCode }: any = await $api('user/auth/reset-password', {
    method: 'POST',
    body: {
      email: form.value.email,
