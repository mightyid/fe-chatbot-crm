...
const startChat = async () => {
  ...
  nextTick(() => {
    ...
    licenseSocket()
    setTimeout(() => {
      console.log(formTest.value, 'formTest.value')
      message.value = mapData(formTest.value)
      sendMessage()
    }, 1000)
  })
}
...
const mapData = (data: any) => {
  ...
  for (const group in data) {
    const trueStatements = Object.entries(data[group])
      .filter(([_, value]) => value === true)
      .map(([statement]) => statement)

    if (trueStatements.length > 0) {
      //@ts-ignore
      filtered[group] = trueStatements
    }
  }

  let message = ''

  Object.entries(filtered).forEach(([group, items]) => {
    message += `${group}:\n`
    //@ts-ignore
    items.forEach((item) => {
      message += `- ${item}\n`
    })
    message += '\n' // thêm dòng trống sau mỗi nhóm
  })
